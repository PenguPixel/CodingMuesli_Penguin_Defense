...
class Projectile {
	...
	lifespan;
	damage;
	hitAudioSrc;    // Geändert: Speichert den Pfad
	hitAudioVolume; // Geändert: Speichert die Lautstärke
	...
	constructor(posX, posY, directionX, directionY, speed = Projectile.DEFAULT_SPEED, imgID = Projectile.DEFAULT_IMG_ID, lifespan = Projectile.DEFAULT_LIFESPAN, damage = Projectile.DEFAULT_DAMAGE) {
		...
		// BUGFIX: Zuweisung korrigiert (Kein Element erstellen, nur die Werte kopieren)
		this.hitAudioSrc = (Math.random() < 0.4) ? Projectile.hitAudio1.src : Projectile.hitAudio2.src;
		this.hitAudioVolume = (Math.random() < 0.4) ? Projectile.hitAudio1.volume : Projectile.hitAudio2.volume;
	}
	...
	playHitSound() {
		// BUGFIX: Schreibfehler "Audiohandle" zu "Audiohandler" korrigiert
